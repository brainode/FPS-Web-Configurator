...
[Authorize]
public sealed class WarsowModel(
    IConfigurationStore configurationStore,
    IDockerAgentClient dockerAgentClient,
    IEnumerable<IGameAdapter> gameAdapters) : PageModel
{
    private readonly IGameAdapter _gameAdapter = gameAdapters.First(a => a.GameKey == "warsow");
    private string GameKey => _gameAdapter.GameKey;

    [BindProperty]
    public InputModel Input { get; set; } = InputModel.FromSettings(new WarsowServerSettings());

    public IReadOnlyList<WarsowGametypeOption> GametypeOptions => WarsowModuleCatalog.Gametypes;
    public IReadOnlyList<WarsowMapGroup> MapGroups => WarsowModuleCatalog.MapGroups;
    public IReadOnlyList<WarsowMapOption> StartMapOptions => Input.SelectedMaps
        .Where(WarsowModuleCatalog.IsValidMap)
        .Distinct(StringComparer.OrdinalIgnoreCase)
        .Select(mapKey => new WarsowMapOption(mapKey))
        .ToArray();

    public ServerStatusSnapshot Status { get; private set; } = ServerStatusSnapshot.NotConfigured("warsow");
    ...
    public string RconPasswordStateHint => HasConfiguredRconPassword
        ? "The current RCON password is shown in the field above. Type a new one to replace it."
        : "Set an RCON password before saving Warsow settings.";
    ...
    private async Task<IActionResult> HandleSaveAsync(bool restartServer, CancellationToken cancellationToken)
    {
        ...
        var existingSettings = WarsowConfigurationSerializer.Deserialize(existingConfiguration.JsonContent);
        ...
        var jsonContent = WarsowConfigurationSerializer.Serialize(effectiveSettings);
        await configurationStore.SaveAsync(GameKey, jsonContent, User.Identity?.Name ?? "unknown", cancellationToken);

        if (!restartServer)
        {
            SuccessMessage = "Warsow settings saved.";
            return RedirectToPage();
        }
    ...
    private async Task LoadAsync(CancellationToken cancellationToken, bool preserveInput = false)
    {
        ...
        var configuration = await configurationStore.GetOrCreateAsync(GameKey, cancellationToken);
        var currentSettings = WarsowConfigurationSerializer.Deserialize(configuration.JsonContent);
    ...
    {
        Input.SelectedMaps = WarsowModuleCatalog.NormalizeMapSelection(Input.SelectedMaps, Input.Gametype, fillDefaultsWhenEmpty);
        Input.StartMap = WarsowModuleCatalog.ResolveStartMap(
            Input.StartMap,
            Input.SelectedMaps,
            Input.Gametype,
    ...
    private void ValidateInput(WarsowServerSettings effectiveSettings)
    {
        if (!WarsowModuleCatalog.IsValidGametype(Input.Gametype))
        {
            ModelState.AddModelError("Input.Gametype", "Choose a supported Warsow game mode.");
        }

        if (!WarsowModuleCatalog.IsValidMap(Input.StartMap))
        {
        ...
        var invalidMaps = Input.SelectedMaps
            .Where(mapKey => !WarsowModuleCatalog.IsValidMap(mapKey))
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToArray();
        ...
        if (string.IsNullOrWhiteSpace(effectiveSettings.RconPassword))
        {
            ModelState.AddModelError("Input.RconPassword", "RCON password is required.");
        }
    }
    ...
    public sealed class InputModel
    {
        ...
        public List<string> SelectedMaps { get; set; } = ["wca1"];
        ...
        public WarsowServerSettings ToSettings(WarsowServerSettings? existingSettings = null)
        {
            return new WarsowServerSettings
            {
                Gametype = Gametype,
                StartMap = StartMap,
        ...
        public static InputModel FromSettings(WarsowServerSettings settings)
        {
